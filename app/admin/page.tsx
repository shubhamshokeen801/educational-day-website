...
export default function AdminDashboard() {
  ...
  const filtered = useMemo(() => {
    ...
    result = result.filter(reg => {
      ...
      const phoneMatch = numericQuery.length > 0 && (
        ...
        members.some(m => m.phone_number?.replace(/\D/g, '').includes(numericQuery))
      );
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    ...
    return (
      <span className={`px-2 py-1 text-xs rounded border ${colors[status as keyof typeof colors] || colors.pending}`}>
        {status}
      </span>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 pt-28">
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                Pending Payment: <span className="font-bold">{pendingPaymentCount}</span>
              </button>
              <button
                onClick={() => {
                  setStatusFilter('all');
            ...
            <div className="relative">
              ...
              >
                <span className="flex-1 text-left text-sm">
                  {selectedEvents.length === 0 ? 'Filter by Events' : `${selectedEvents.length} events`}
                </span>
                <ChevronRight className={`w-4 h-4 transition-transform ${showEventDropdown ? 'rotate-90' : ''}`} />
              </button>
                  ...
                  <div className="absolute z-20 mt-1 w-64 bg-white border border-gray-300 rounded-lg shadow-lg max-h-80 overflow-auto">
                    {selectedEvents.length > 0 && (
                      <div className="p-2 border-b bg-gray-50">
                        <button
                          onClick={() => { setSelectedEvents([]); setCurrentPage(1); }}
              ...
              <tbody>
                {loading ? (
                  ...
                  paginatedData.map((reg, idx) => {
                    ...
                    const hasTeamMembers = reg.team_id && reg.team_members && reg.team_members.length > 0;
                      ...
                      <tr key={reg.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="px-3 py-3">
                          <div className="flex flex-col gap-1">
                            ...
                            <span className="text-xs text-gray-500 uppercase">{reg.event_type}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-3 py-3 text-center">
                          {hasTeamMembers ? (
                            ...
                            >
                              ...
                              <span className="text-xs">({reg.team_members!.length})</span>
                            </button>
                          ) : '—'}
                        </td>
                        <td className="px-3 py-3 text-center">
                          {isFree ? (
                            ...
                            <button
                              onClick={() => openImageModal(reg.payment_proof_url!)}
                        ...
                        <td className="px-3 py-3">
                          {isFree ? (
                            ...
                            <select
                              value={reg.payment_verification}
                              onChange={e => updateStatus(reg.id, 'payment_verification', e.target.value)}
      ...
      {/* Team Members Modal */}
      {showTeamModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4" onClick={() => setShowTeamModal(false)}>
